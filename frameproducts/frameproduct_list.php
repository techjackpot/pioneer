...
if ($_GET['export'] === 'csv') {
	$data = FrameProduct::find( ['limit' => null, 'total' => false] + $table->find_params() );
	$csv = new CSV();
	$csv->conditional_columns( array_keys($export_fields) );
	$csv->output($data, $identifier);
}
else {
	$data = FrameProduct::find( $table->find_params() );
	...
	<table class="table table-striped">
		<thead>
			<tr>
				...
				<th><?= $table->header('turnaround_days') ?></th>
				<th><?= $table->header('quantity_limit') ?></th>
...
<?php foreach($data as $index => $model): ?>
			<tr>
				<td><a href="<?=admin_url($identifier.'s')?>/edit/<?=$model['id']?>"><?= h($model->display('name')) ?></a></td>
				<td><?= h($model->display('turnaround_days')) ?></td>
				<td><?= h($model->display('quantity_limit')) ?></td>
