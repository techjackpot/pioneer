...
if ($path[2] === 'delete') {
	...
	if ($_POST) {
		$values = $form->apply_post('name', 'turnaround_days', 'quantity_limit');
	...
	<?php
		...
		$form->field('turnaround_days'),
		$form->field('quantity_limit'),
		$form->submit(),
		$form->end();	
	}
	else { // Add
