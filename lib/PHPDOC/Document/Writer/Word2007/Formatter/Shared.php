...
class Shared
{
    ...
    {
    	if (!($val instanceof Metric)) $val = new Metric($val, Metric::UNIT_DXA);
    	if ($val->isUnitRelative()) $val->convertTo(Metric::UNIT_PCT);
    	else $val->convertTo(Metric::UNIT_DXA);
        $type = $val->getUnit();
        $w = $val->getValueForXML();
    ...
    {
        if (is_bool($val)) {
            $val = $this->getOnOff($val);
        }
        if ($val !== null and $val !== '') {
            $node = $root->ownerDocument->createElement('w:' . $name);
            if (is_array($val)) $this->setAttributesToNode($val, $node);
            else $node->appendChild(new \DOMAttr($key, $val));
            $root->appendChild($node);
        }
        return true;
    }

	public function setAttributesToNode(array $attrs, \DOMNode $node, $prefix = null)
	{
		foreach ($attrs as $key=>$value) {
            if ($value === null) continue;
            if (is_bool($value)) $value = $this->getOnOff($value);
            $node->appendChild(new \DOMAttr($prefix.$key, $value));
		}
	}
