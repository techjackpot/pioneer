...
class Word2007 implements WriterInterface
{
    ...
    public function save($output = null)
    {
        ...
        $this->processDocumentExtras($root);
    ...
    /**
     * Process additional w:document properties (background)
     *
     * @param \DOMNode  $root    The root node to update
     * @return \DOMNode Returns the new or original root node
     */
    private function processDocumentExtras(\DOMNode $root)
    {
    	if ($this->document === null) {
            throw new SaveException("No document defined");
        }
        $p = $this->document->getProperties();
        if (!$p->has('background')) return $root;
        $bg = $p->get('background');
        if (!preg_match("/^[0-9A-Fa-f]{6}$/", $bg)) {
            if (!is_numeric($bg)) return true;
            $val = sprintf('%06x', $bg);
        }
        $prop = $root->ownerDocument->createElement('w:background');
        $prop->appendChild(new \DOMAttr('w:color', $bg));
    ...
    private function processListItemsElement(ElementInterface $element, \DOMNode $root)
    {
        ...
        foreach ($element as $item) {
            ...
            if ( $prop->get('ovrd_level') ) $prop['numPr.ilvl'] = $prop->get('ovrd_level');
            else $prop['numPr.ilvl'] = $level;
    ...
    private function processParagraphElement(ElementInterface $element, \DOMNode $root)
    {
        ...
        if ($this->formatter->format($element, $prop)) {
            $run = $element->getProperties()->get('run');
            if ($run) {
	            $e = new TextRun(null, $run);
	            $rprop = $dom->createElement('w:rPr');
	            if ($this->formatter->format($e, $rprop)) $prop->appendChild($rprop);
            }
            $node->appendChild($prop);
        }
    ...
    private function processTableElement(ElementInterface $element, \DOMNode $root)
    {
        ...
        if ($grid) {
            ...
            foreach ($grid as $w) {
                ...
                $col->appendChild(new \DOMAttr('w:w', Metric::make($w)->convertTo(Metric::UNIT_DXA)));
...
//        $node->appendChild($dom->createElement('w:lastRenderedPageBreak'));
        $root->appendChild($node);
     ...
     */
    private function processImageElement(ImageInterface $element, \DOMNode $root)
    {
        static $imgId = 0;
        ...
        $rect->appendChild(new \DOMAttr('style', $element->getShapeStyle($this->document)));
	...
	private function processTableStyle(StyleInterface $style, \DOMNode $root)
	{
		$dom = $root->ownerDocument;

		$root->appendChild(new \DOMAttr('w:type', 'table'));
		$root->appendChild(new \DOMAttr('w:customStyle', '1'));

		foreach ($style->getAllStyles() as $type=>$props) {
			if ($type == \PHPDOC\Style\TableStyle::TYPE_DEFAULT) {
				$node = $root;
			} else {
				$node = $dom->createElement('w:tblStylePr');
				$node->appendChild(new \DOMAttr('w:type', $type));
				$root->appendChild($node);
			}

			$element = new Table($props);
			$element->getProperties()
				->remove('look');
			$prNode = $dom->createElement('w:tblPr');
			$this->formatter->format($element, $prNode);
			$node->appendChild($prNode);
		
			foreach (
				array(
					'paragraph' => array('pPr', 'Paragraph'),
					'run' => array('rPr', 'TextRun'),
					'cell' => array('tcPr', 'TableCell'),
					'row' => array('trPr', 'TableRow')
				) as $key=>$e
			)
			{
				if (!$props->has($key)) continue;
				$p = $props->get($key);
				$class = '\\PHPDOC\\Element\\'.$e[1];
				$element = new $class($p);
				$prNode = $dom->createElement('w:' . $e[0]);
				$node->appendChild($prNode);
				$this->formatter->format($element, $prNode);
			}
		}
